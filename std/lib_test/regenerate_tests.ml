...
let regenerate_tests compile =
  let repeat i j re =
    ...
    | Some j ->
        for _ = succ i to j do
          r := Regex.seq !r (Regex.opt re)
        done);
    !r
  in
  let alphabet = [ 'a'; 'b'; 'c'; 'd'; 'e'; 'f' ] in
  let allof cs =
    List.fold_right (fun c r -> Regex.(alt (chr c) r)) cs Regex.empty
  in
  ...
  let rec to_re =
    ...
    | And _ | Not _ ->
        (* No support for intersection or complement at present *)
        QCheck.assume_fail ()
...
let () =
  print_string "regenerate tests (NFA)...";
  flush stdout;
  regenerate_tests
    (combinator_matcher ~compile:Regex.compile ~accept:nfa_accept);
  print_endline "OK!";

  print_string "regenerate tests (DFA)...";
  flush stdout;
  regenerate_tests (combinator_matcher ~compile:dfa_compile ~accept:dfa_accept);
  print_endline "OK!";

  print_string "regenerate tests (DFA-minimized)...";
  flush stdout;
  regenerate_tests
    (combinator_matcher ~compile:dfa_minimize_compile ~accept:dfa_accept);
  print_endline "OK!"
